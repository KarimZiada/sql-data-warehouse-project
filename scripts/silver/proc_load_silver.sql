...
BEGIN
DECLARE @start_time DATETIME, @end_time DATETIME, @start_b DATETIME, @end_b DATETIME;
    BEGIN TRY
        SET @start_b = GETDATE();
        PRINT '==========================================';
        ...
        END sls_order_dt,
        CASE WHEN sls_ship_dt < 0 OR LEN(sls_ship_dt) != 8 OR sls_ship_dt < sls_order_dt then NULL
            ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
        END sls_ship_dt,
        CASE WHEN sls_due_dt < 0 OR LEN(sls_due_dt) != 8 OR sls_due_dt < sls_order_dt then NULL
            ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
        END sls_due_dt,
        CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price) then sls_price * sls_quantity
            ELSE sls_sales
        END sls_sales,
        sls_quantity,
        ...
        PRINT '----------------'
        PRINT '>> Silver Layer Loading Time: ' + CAST(DATEDIFF(SECOND, @start_b, @end_b) AS NVARCHAR) + 'seconds';
        PRINT '----------------';
    END TRY

    BEGIN CATCH
        PRINT '=====================================================';
        PRINT 'ERROR OCCURED DURING LOADING SILVER LAYER' PRINT 'Error Message' + ERROR_MESSAGE();
